...
var Varity = exports.Varity = function Varity () {
  ...
  _(this).bindAll('buildExpectations', 'buildEvaluator', 'buildParams');
};

module.exports = Varity;
...
Varity.prototype.buildEvaluator = function(context) {
  ...
  this.expectations.push(function(arg, next) {
    ...
    var res = _(self.getOperations(context)).reduce(function(memo, operation) {
      ...
      return memo;
    }, self.thingOrDefault(arg, context.types));
    return res;
  });
};
...
Varity.prototype.thingOrDefault = function(actual, expected) {
  if (~expected.indexOf(_(actual).stringify())) {
    return actual;
  } else {
    if (~this.options.populate.indexOf(expected[0])) {
...
Varity.prototype.buildParams = function(args) {
  ...
  return _(self.expectations).reduce(function(memo, expectation) {
    memo.push(expectation(args.shift(), args[0]));
    return memo;
  }, []);
};
